...
impl Shared {
    fn new() -> Self {
        Self {
            ...
            queued_packets: std::array::from_fn(|_| AtomicU64::new(0)),
    ...
    fn health(&self, stopped: bool) -> AppResult<RecordingHealth> {
        let ended = self.stopped_ms.load(Ordering::Acquire);
        let wall = if ended == 0 {
            self.started.elapsed().as_secs_f64()
        } else {
            (ended - 1) as f64 / 1_000.0
        };
...
fn queue_packet(sender: &SyncSender<Packet>, packet: Packet, shared: &Shared) -> AppResult<()> {
    ...
    let queued = &shared.queued_packets[source_index(source)];
    ...
    match sender.try_send(packet) {
        ...
        Err(TrySendError::Full(_)) => {
            ...
            shared.warn(format!(
                "{} capture queue filled; some audio was lost. Available sections were kept.",
                source.label()
            ));
            Ok(())
        }
        Err(TrySendError::Disconnected(_)) => {
            ...
            Err(AppError::new(
                "audio_capture",
                "Audio file writer stopped unexpectedly",
            ))
        }
    }
}
...
impl SourceWriter {
    ...
    fn open(&mut self, start_frame: u64) -> AppResult<()> {
        self.path = if self.segmented {
            segment_path(&self.base, self.source, self.index)?
        } else {
            self.base.clone()
        };
    ...
    fn write(&mut self, packet: Packet, shared: &Shared) -> AppResult<()> {
        if self.failed || packet.pcm.is_empty() {
            return Ok(());
        }
        if packet.pcm.len() % 2 != 0 {
            return Err(AppError::new(
                "audio_capture",
                "An audio packet ended inside a PCM frame",
            ));
        }
        ...
        if let Some(expected) = self
            .next_frame
            .or(if self.segmented { None } else { Some(0) })
        {
            if start.abs_diff(expected) <= TIMESTAMP_TOLERANCE {
                start = expected;
            } else if start > expected {
                shared.warn(format!(
                    "{} capture contained a gap. Section timestamps preserve the missing interval.",
                    self.source.label()
                ));
                if self.segmented {
                    self.finish(shared)?;
                    ...
                    while missing > 0 {
                        ...
                        self.writer
                            ...
                            .write_pcm(&silence[..frames as usize * 2])?;
                        missing -= frames;
                    }
                }
            } else {
                shared.warn(format!(
                    "{} capture timestamps overlapped. Repeated frames were omitted.",
                    self.source.label()
                ));
        ...
        while !pcm.is_empty() {
            ...
            let available = if self.segmented {
                SECTION_FRAMES - writer.frames()
            } else {
                u64::MAX
            };
            ...
            counter
                ...
                .store(packet.captured_ms.saturating_add(1), Ordering::Release);
            pcm = &pcm[frames * 2..];
            start += frames as u64;
            self.next_frame = Some(start);
            if self.segmented && writer.frames() == SECTION_FRAMES {
                self.finish(shared)?;
            }
        }
        Ok(())
    ...
    fn fail(&mut self, shared: &Shared, error: AppError) {
        ...
        shared.warn(format!(
            "{} could not save an audio section: {}. Captured files were kept.",
            self.source.label(),
            error.message
        ));
    }
...
impl RecordingWriter {
    fn new(system: PathBuf, microphone: PathBuf, segmented: bool, shared: Arc<Shared>) -> Self {
        Self {
            sources: [
                ...
                SourceWriter::new(
                    if segmented { system } else { microphone },
    ...
    fn write_packet(&mut self, packet: Packet) {
        ...
        if let Err(error) = source.write(packet, &self.shared) {
            source.fail(&self.shared, error);
        }
        self.rotate_due();
    }
    ...
    fn rotate_due(&mut self) {
        ...
        for source in &mut self.sources {
            // A different source can keep the queue busy while this endpoint is quiet. Check
            // every pass, but avoid retiring old partial packets while this source catches up.
            let caught_up = self.shared.queued_packets[source_index(source.source)]
                .load(Ordering::Acquire)
                == 0;
            if source.segmented
                && source.writer.is_some()
                && caught_up
                && now.saturating_sub(source.start_frame) >= SECTION_FRAMES
            {
                if let Err(error) = source.finish(&self.shared) {
                    source.fail(&self.shared, error);
                }
            }
        }
    ...
    fn run(mut self, receiver: Receiver<Packet>) {
        loop {
            match receiver.recv_timeout(Duration::from_millis(200)) {
                Ok(packet) => self.accept_queued_packet(packet),
                Err(mpsc::RecvTimeoutError::Timeout) => self.rotate_due(),
    ...
    fn accept_queued_packet(&mut self, packet: Packet) {
        self.shared.queued_packets[source_index(packet.source)].fetch_sub(1, Ordering::AcqRel);
...
mod platform {
    ...
    fn windows_error(error: windows::core::Error) -> AppError {
        AppError::new(
            "audio_capture",
            format!("Windows audio error 0x{:08X}", error.code().0 as u32),
        )
    }
    ...
    fn qpc_time() -> AppResult<u64> {
        ...
        if ticks < 0 || frequency <= 0 {
            return Err(AppError::new(
                "audio_capture",
                "Windows returned an invalid capture clock",
            ));
        }
    ...
    impl Stream {
        fn open(enumerator: &IMMDeviceEnumerator, source: AudioSource) -> AppResult<Self> {
            let direction = if source == AudioSource::System {
                eRender
            } else {
                eCapture
            };
            ...
            unsafe {
                let device = enumerator
                    .GetDefaultAudioEndpoint(direction, eConsole)
                    .map_err(windows_error)?;
                // On initialization failure, release the client before closing its event.
                let event = Event::new()?;
                let client: IAudioClient =
                    device.Activate(CLSCTX_ALL, None).map_err(windows_error)?;
                client
        ...
        fn start(&mut self) -> AppResult<()> {
            unsafe {
                self.client.Start().map_err(windows_error)?;
            }
            self.started = true;
            Ok(())
        }
        ...
        fn stop(&mut self) -> AppResult<()> {
            if self.started {
                self.started = false;
                unsafe {
                    self.client.Stop().map_err(windows_error)?;
                }
            }
            Ok(())
        }
        ...
        fn drain(
            ...
            for _ in 0..32 {
                ...
                let pcm = if frames > SAMPLE_RATE * 2 {
                    Err(AppError::new(
                        "audio_capture",
                        "Windows returned an oversized audio packet",
                    ))
                    ...
                    Err(AppError::new(
                        "audio_capture",
                        "Windows returned an invalid audio buffer",
                    ))
                } else {
                ...
                {
                    shared.warn(format!(
                        "{} reported an audio discontinuity; some speech may be missing.",
                        self.source.label()
                    ));
                }
                let captured_ms = shared.started.elapsed().as_millis() as u64;
                let start_frame = if flags & AUDCLNT_BUFFERFLAGS_TIMESTAMP_ERROR.0 as u32 != 0
                    || timestamp < origin
                {
    ...
    fn run_capture(
        ...
        // Field order releases all COM interfaces before uninitializing their owner apartment,
        // including early returns and unwinding.
        struct Capture {
            streams: Vec<Stream>,
            _apartment: Apartment,
        }
        let setup = || -> AppResult<Capture> {
            ...
            if streams.is_empty() {
                return Err(AppError::new("audio_capture", "Neither Windows audio source could start. Check audio devices and microphone permissions."));
            }
            Ok(Capture {
                streams,
                _apartment: apartment,
        ...
        while !shared.stopping.load(Ordering::Acquire) && !capture.streams.is_empty() {
            ...
            capture.streams.retain_mut(|stream| match stream.drain(&sender, &shared, origin) {
                Ok(()) => true,
                Err(error) => {
        ...
        for stream in &mut capture.streams {
            if let Err(error) = stream
                .stop()
                .and_then(|_| stream.drain(&sender, &shared, origin))
            {
                shared.warn(format!(
                    "{} could not finish capture: {}. Available audio was kept.",
                    stream.source.label(),
                    error.message
                ));
            }
    ...
    impl NativeRecording {
        pub(crate) fn start(system: &Path, microphone: &Path, segmented: bool) -> AppResult<Self> {
            if system
                .extension()
                .map_or(true, |extension| extension != "wav")
                || microphone
                    .extension()
                    .map_or(true, |extension| extension != "wav")
            {
                return Err(AppError::new(
                    "invalid_audio_path",
                    "Windows capture requires WAV paths",
                ));
            }
            ...
            recording.writer = Some(
                thread::Builder::new()
                    .name("audio-files".into())
                    .spawn(move || {
            ...
            recording.capture = match thread::Builder::new()
                .name("wasapi-capture".into())
                .spawn(move || run_capture(shared, sender, ready_sender, origin))
            {
                Ok(thread) => Some(thread),
                Err(error) => return Err(AppError::new("audio_capture", error.to_string())),
            };
            ready_receiver.recv().map_err(|_| {
                AppError::new("audio_capture", "Windows capture stopped during startup")
            })??;
            Ok(recording)
        }

        pub(crate) fn rotate(&mut self) -> AppResult<Vec<CapturedSegment>> {
            self.shared.take_segments()
        }
        pub(crate) fn health(&self) -> AppResult<RecordingHealth> {
            self.shared.health(false)
        }

        fn shutdown(&mut self) {
            let ended = (self.shared.started.elapsed().as_millis() as u64).saturating_add(1);
            let _ = self.shared.stopped_ms.compare_exchange(
                0,
                ended,
                Ordering::AcqRel,
                Ordering::Acquire,
            ...
            for worker in [&mut self.capture, &mut self.writer] {
                if let Some(worker) = worker.take() {
                    if worker.join().is_err() {
                        self.shared.warn(
                            "An audio worker stopped unexpectedly. Available files were kept."
                                .into(),
                        );
                    }
                }
        ...
        pub(crate) fn stop(mut self) -> AppResult<RecordingFiles> {
            ...
            Ok(RecordingFiles {
                ...
                health: Some(self.shared.health(true)?),
                segments: self.shared.take_segments()?,
...
mod tests {
    ...
    impl Directory {
        fn new() -> Self {
            let path =
                std::env::temp_dir().join(format!("meeting-windows-{}", uuid::Uuid::new_v4()));
    ...
    fn automatic_rotation_keeps_every_frame_and_final_partial_section() {
        ...
        assert_eq!(
            crate::audio::inspect(&first[0].path).unwrap().frames,
            960_000
        );
        assert_eq!(crate::audio::inspect(&tail[0].path).unwrap().frames, 16_000);
        assert_eq!(
            shared.counters[1].written_frames.load(Ordering::Acquire),
            976_000
        );
    }
    ...
    fn a_capture_gap_preserves_the_next_sections_true_time() {
        ...
        assert!(shared
            ...
            .warnings
            .iter()
            .any(|warning| warning.contains("gap")));
    }

    #[test]
    fn a_gap_after_a_full_section_is_preserved_and_reported() {
        let directory = Directory::new();
        let shared = Arc::new(Shared::new());
        let mut writer = writer(&directory, shared.clone());
        writer.write_packet(packet(AudioSource::Microphone, 0, SECTION_FRAMES as usize));
        writer.write_packet(packet(
            AudioSource::Microphone,
            65 * u64::from(SAMPLE_RATE),
            16,
        ));
        writer.finish();
        let sections = shared.take_segments().unwrap();
        assert_eq!(sections.len(), 2);
        assert_eq!(sections[1].start_seconds, 65.0);
        assert!(shared
            .health(false)
            .unwrap()
            .warnings
    ...
    #[test]
    fn tiny_timestamp_rounding_at_rotation_does_not_drop_audio() {
        let directory = Directory::new();
        let shared = Arc::new(Shared::new());
        let mut writer = writer(&directory, shared.clone());
        writer.write_packet(packet(AudioSource::Microphone, 0, SECTION_FRAMES as usize));
        writer.write_packet(packet(AudioSource::Microphone, SECTION_FRAMES - 1, 16));
        writer.finish();
        let sections = shared.take_segments().unwrap();
        assert_eq!(sections[1].start_seconds, 60.0);
        assert_eq!(crate::audio::inspect(&sections[1].path).unwrap().frames, 16);
    }

    #[test]
    fn a_quiet_source_finalizes_while_the_other_source_keeps_writing() {
        let directory = Directory::new();
        ...
        writer.accept_queued_packet(receiver.recv().unwrap());
        queue_packet(
            &sender,
            packet(AudioSource::Microphone, 61 * u64::from(SAMPLE_RATE), 16),
            &shared,
        )
        .unwrap();
        writer.accept_queued_packet(receiver.recv().unwrap());
        ...
        assert_eq!(sections[0].source, AudioSource::System);
        assert!(writer.sources[1].writer.is_some());
        writer.finish();
    }
    ...
    fn queue_overflow_is_visible_and_is_not_counted_as_written_audio() {
        ...
        assert!(health
            .warnings
            .iter()
            .any(|warning| warning.contains("queue")));
    }
    ...
    fn writer_drains_packets_before_finalizing_on_stop() {
        ...
        assert_eq!(sections.len(), 1);
        assert_eq!(
            crate::audio::inspect(&sections[0].path).unwrap().frames,
            300
        );
