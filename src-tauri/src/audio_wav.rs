...
fn invalid(message: impl Into<String>) -> AppError {
    AppError::new("audio_chunk", message)
}
...
impl Writer {
    pub(crate) fn create(path: &Path, sample_rate: u32) -> AppResult<Self> {
        if !(8_000..=192_000).contains(&sample_rate) {
            return Err(invalid("Unsupported PCM sample rate"));
        }
    ...
    pub(crate) fn write_pcm(&mut self, pcm: &[u8]) -> AppResult<()> {
        ...
        if pcm.len() % 2 != 0 || size.is_none() {
            return Err(invalid("Invalid or oversized PCM recording"));
        }
    ...
    pub(crate) fn finish(mut self) -> AppResult<()> {
        if self.file.metadata().map_err(io_error)?.len() != 44 + u64::from(self.bytes) {
            return Err(invalid(
                "A partial disk write left this WAV unfinished. The audio was kept.",
            ));
        }
...
fn parse(reader: &mut (impl Read + Seek)) -> AppResult<PcmInfo> {
    ...
    while position < length {
        ...
        let end = start + size;
        if end > length || end + size % 2 > length {
            return Err(invalid("Truncated WAV audio"));
        }
        match &chunk[..4] {
            b"fmt " => {
                if sample_rate.is_some() || size != 16 {
            ...
            b"data" => {
                if data.is_some() || size % 2 != 0 {
                    return Err(invalid("Invalid WAV sample data"));
                }
...
pub(crate) fn write_chunk(input: &Path, output: &Path, start: f64, duration: f64) -> AppResult<()> {
    ...
    if first >= pcm.info.frames as f64
        || !first.is_finite()
        || frames < 1.0
    ...
    if frames * 2 + 44 >= 24_000_000 {
        return Err(AppError::new(
            "audio_chunk_size",
            "Audio chunk exceeds the safe upload size",
        ));
    }
    source
        .seek(SeekFrom::Start(pcm.data_start + first as u64 * 2))
        .map_err(io_error)?;
...
mod tests {
    ...
    fn durable_wave_chunks_preserve_exact_samples_and_tail() {
        ...
        let samples: Vec<u8> = (0..32_123)
            .flat_map(|frame| ((frame % 30_000) as i16).to_le_bytes())
            .collect();
    ...
    fn rejects_truncation_malformed_format_and_unfinished_audio() {
        ...
        for length in 0..valid.len() {
            assert!(
                inspect_bytes(&valid[..length]).is_err(),
        ...
        for offset in [0, 4, 8, 12, 16, 20, 22, 24, 28, 32, 34, 36, 40] {
            ...
            assert!(
                inspect_bytes(&malformed).is_err(),
                "accepted corrupt field {offset}"
            );
        }
    ...
    fn bounds_ranges_and_refuses_oversized_upload_without_output() {
        ...
        for _ in 0..300 {
            writer.write_pcm(&vec![0; 96_000]).unwrap();
        }
        writer.finish().unwrap();
