...
fn followup_questions_keep_history_separate_from_current_evidence() {
    ...
    let history = [QuestionTurn {
        question: "When is the review?".into(),
        answer: "It was Friday.".into(),
    }];
    ...
    let answer =
        tauri::async_runtime::block_on(OpenAiClient::with_base_url(url).ask_meetings_with_history(
            &[source],
            "Has that changed?",
            "test-key",
            &history,
    ...
    let input: Value =
        serde_json::from_str(request["input"][0]["content"][0]["text"].as_str().unwrap()).unwrap();
    assert_eq!(input["conversation"][0]["answer"], "It was Friday.");
    assert!(!input["passages"].to_string().contains("It was Friday."));
    let legal_ids = request["text"]["format"]["schema"]["properties"]["source_ids"]["items"]
        ["enum"]
        .as_array()
        .unwrap();
    assert!(legal_ids.contains(&json!("m0:notes:0")));
    assert!(legal_ids.iter().all(|id| input["passages"]
        .as_array()
        .unwrap()
        .iter()
        .any(|passage| &passage["id"] == id)));
}
...
fn excessive_question_history_is_rejected_before_a_paid_request() {
    ...
    let error = tauri::async_runtime::block_on(
        OpenAiClient::with_base_url("http://127.0.0.1:9/v1").ask_meetings_with_history(
            &[session()],
            "And then?",
            "test-key",
            &history,
...
fn wav_upload_uses_pcm_mime_and_rejects_truncation_before_network() {
    ...
    std::fs::write(&path, &audio).unwrap();
    let error = tauri::async_runtime::block_on(
        OpenAiClient::with_base_url("http://127.0.0.1:1/v1").transcribe(&path, "test-key"),
    )
    .unwrap_err();
    ...
    std::fs::write(&path, &audio).unwrap();
    let empty = tauri::async_runtime::block_on(
        OpenAiClient::with_base_url("http://127.0.0.1:1/v1").transcribe(&path, "test-key"),
    )
    .unwrap();
...
fn live_synthetic_meeting_questions_with_openai() {
    ...
    let followup = tauri::async_runtime::block_on(client.ask_meetings_with_history(
        &[source.clone()],
        "Who will request that report, and when? Answer entirely in Spanish.",
        key.trim(),
        &[meeting_notes_lib::domain::QuestionTurn {
            question: "what was it about".into(),
            answer: answer.answer.clone(),
        }],
    ))
    .unwrap();
    assert!(followup.answer.to_lowercase().contains("alex"));
    assert!(followup.answer.to_lowercase().contains("viernes"));
    assert!(!followup.citations.is_empty());
    for citation in &followup.citations {
        assert!(
            source.notes().contains(&citation.excerpt)
                || source
                    .transcript
    ...
    println!("Synthetic text-only acceptance: broad summary with {} saved passages; contextual Spanish follow-up with {} saved passages; unavailable fact handled normally; three requests in {:.1}s.", answer.citations.len(), followup.citations.len(), started.elapsed().as_secs_f64());
}
